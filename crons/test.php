<?php
	...
	if( !$database->connect_error ){		
		...
		//Update User Values
		$database->query( "UPDATE users_rounds SET food = food + ( food_income - food_upkeep ), stone = stone + ( stone_income - stone_upkeep ), wood = wood + ( wood_income - wood_upkeep ), faith = faith + ( faith_income - faith_upkeep ), mana = mana + ( mana_income - mana_upkeep ), gold = gold + ( gold_income - gold_upkeep )" );						
		...
		$rounds = $database->query( "SELECT id, max_turns, turns FROM rounds WHERE active = 1" );
		if( $rounds && $rounds->num_rows > 0 ) {
			while( $round = $rounds->fetch_object() ) {
				$database->query ( "DELETE FROM rankings WHERE roundid = 0" );
				//Update Users turns
				$database->query( "UPDATE users_rounds SET turns = $round->max_turns WHERE turns < $round->max_turns AND turns > $round->max_turns - $round->turns" );
				$database->query( "UPDATE users_rounds SET turns = turns + $round->turns WHERE turns < $round->max_turns AND roundid = $round->id" );
